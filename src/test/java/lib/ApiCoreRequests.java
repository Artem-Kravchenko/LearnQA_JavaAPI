...
public class ApiCoreRequests {
    ...
    @Step("Make a DELETE-request to delete existing user with token and cookie")
    public Response makeDeleteRequestWithTokenAndCookie(String url, String token, String cookie) {
        return given()
                .filter(new AllureRestAssured())
                .header(new Header("x-csrf-token", token))
                .cookie("auth_sid", cookie)
